...
- **Shared partitions (`small`, `longrun`):** limits are per *job*, so take a partial node.
  ...
  memory with `--mem=` / `--mem-per-cpu=`.`--mem-per-cpu × --cpus-per-task` should equal the total memory reserved. 
...
- **High-memory partitions (`hugemem`, `hugemem_longrun`):** Use only when the job genuinely
  needs >1500 GiB. For full node jobs on these nodes, keep `ntasks-per-node × cpus-per-task = 128`.
  Otherwise they behave same as shared partitions.
...
- **Local Storage:** 
  - Use `$TMPDIR` for I/O intensive tasks, 20GiB quota on small, interactive partitions. 600GiB on full nodes, and 150GiB on GPU nodes.
  - *Important:* You must move data back to shared storage before the script ends.
- **Fast Local Scratch:** Use `--exclusive` and the `#SBATCH --bb="#BB_LUA SBF storagesize=<local_storage_space> path=/run/sbb/<username>"` directive for network-based fast local storage.
